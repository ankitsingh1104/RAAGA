...
const Albums = () => {
    ...
    Promise.all(
      genreQueries.map(({ key, query }) =>
        fetchYouTubeData(query, 4, controller.signal)
          ...
          .catch(() => ({ key, tracks: [] as Track[] }))
      )
    ).then((results) => {
      const genreMap: Record<string, Track[]> = {};
      results.forEach(({ key, tracks }) => { genreMap[key] = tracks; });
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      results.forEach(({ key, tracks }) => { moodMap[key] = tracks; });
      setMoodData(moodMap);
  ...
  return (
    <main className="pb-8">
          ...
          <div className="flex gap-4 px-6 pb-4">
            {genreLoading
              ...
              : genreQueries.map(({ key, title }) => (
                  <GenreCard key={key} title={title} image={genreData[key]?.[0]?.cover || "/placeholder.svg"} tracks={genreData[key] || []} />
                ))}
          </div>
          ...
          <div className="flex gap-4 px-6 pb-4">
            {moodLoading
              ...
              : moodQueries.map(({ key, title }) => (
                  <MoodPlaylistCard key={key} title={title} image={moodData[key]?.[0]?.cover || "/placeholder.svg"} tracks={moodData[key] || []} />
                ))}
          </div>
          <ScrollBar orientation="horizontal" />
        </ScrollArea>
      ...
      <SectionCarousel title="Popular Artists" highlightedWord="Artists" tracks={data.artists || []} isLoading={loading.artists} variant="round" />
      <SectionCarousel title="Music Video" highlightedWord="Video" tracks={data.videos || []} isLoading={loading.videos} variant="video" />
      <SectionCarousel title="New Release Songs" highlightedWord="Songs" tracks={data.newRelease || []} isLoading={loading.newRelease} variant="default" />
      <SectionCarousel title="Top Albums" highlightedWord="Albums" tracks={data.topAlbums || []} isLoading={loading.topAlbums} variant="album" />
